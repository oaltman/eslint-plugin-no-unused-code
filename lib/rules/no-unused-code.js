...
module.exports = {
    meta: {
        docs: {
            description: "Custom - disallow unused variables",
            category: "Variables",
            recommended: true
        },
        fixable: "code",
    ...
    create(context) {
        ...
        //--------------------------------------------------------------------------
				const cases = {
					funParam: require('./cases/fun-param'),
					funParamDestr: require('./cases/fun-param-destr'),
					fun: require('./cases/fun'),
					property1: require('./cases/object-property'),
					property2: require('./cases/object-property-2'),
					array: require('./cases/array'),
					variableDeclaration: require('./cases/variable-declaration'),
				}
				const pickCase = function(typeCase, fixer, unusedVar) {
					// console.log(typeCase, ':', unusedVar.identifiers[0].name)
					return cases[typeCase](fixer, unusedVar, fix)
				}

				function fix(fixer, unusedVar) {
					// FUNCTION PARAMETERS
					if (unusedVar.defs[0].type === 'Parameter') {
						if(unusedVar.identifiers[0].parent.type === "AssignmentPattern") {
							if (unusedVar.identifiers[0].parent.parent.parent.type === 'ObjectPattern') {
								return pickCase('funParamDestr', fixer, unusedVar)
							}
						}
						if (unusedVar.identifiers[0].parent.parent.type === 'ObjectPattern') {
							return pickCase('funParamDestr', fixer, unusedVar)
						}
						return pickCase('funParam', fixer, unusedVar)
					}
					// FUNCTION NAME
					if (unusedVar.defs && unusedVar.defs[0].type === 'FunctionName') {
						return pickCase('fun', fixer, unusedVar)
					}
					// PROPERTY INSIDE OBJECT
					if (unusedVar.parent && unusedVar.parent.type === 'Property') {
						return pickCase('property1', fixer, unusedVar)
					}
					// PROPERTY INSIDE OBJECT
					if (unusedVar.identifiers[0].type === 'Identifier' && unusedVar.identifiers[0].parent.type === 'Property') {
						return pickCase('property2', fixer, unusedVar)
					}
					// VARIABLE DECLARATION // const unused = 1
					if (unusedVar.identifiers[0].type === 'Identifier' && unusedVar.identifiers[0].parent.type === 'VariableDeclarator') {
						return pickCase('variableDeclaration', fixer, unusedVar)
					}
					if (unusedVar.identifiers[0].parent.type === 'ArrayPattern') {
						return pickCase('array', fixer, unusedVar)
					}
					return
				}
